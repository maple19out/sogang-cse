...
process_execute (const char *file_name) 
{
  ...
  strlcpy (fn_copy, file_name, PGSIZE);
	...
	for(i=0; file_name[i]!=' ' && file_name[i]!= '\0'; i++)
		command_line[i] = file_name[i];
	command_line[i] = '\0';


  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (command_line, PRI_DEFAULT, start_process, fn_copy);
  if (tid == TID_ERROR)
...
start_process (void *file_name_)
{
  ...
  t->load_succeded = success = load (file_name, &if_.eip, &if_.esp);
...
process_wait (tid_t child_tid) 
{
	...
	t = get_child_thread(child_tid);
	if(t!=NULL) {
		sema_down(&(t->exit_lock));
...
process_exit (void)
{
  ...
  if (pd != NULL) 
	...
	for(int i=0; i<131; i++) {
		if(cur->_file[i]!=NULL)
			file_close(cur->_file[i]);
	}

	sema_up(&(cur->exit_lock));
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  process_activate ();
	...
	//Required implementation : parsing arguments
	char command_line[1024];
	char **argv;
	char *token;
	char *next_ptr;
	...
	strlcpy(command_line, file_name, strlen(file_name)+1);
	token = strtok_r(command_line, " ", &next_ptr);
	...
	strlcpy(command_line, file_name, strlen(file_name)+1);
	argv[0] = strtok_r(command_line, " ", &next_ptr);

	for(int i=1; i<argc; i++)
		argv[i] = strtok_r(NULL, " ", &next_ptr);
  ...
  if (!setup_stack (esp))
	...
	//Required implementation : pushing arguments on stack
	int word_align=0;
	for(int i=argc-1; i>=0; i--) {
		int argv_size = strlen(argv[i])+1;

		*esp -= argv_size;
		word_align+=argv_size;
		
		strlcpy(*esp, argv[i], argv_size);
		argv[i] = *esp;
	}
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      size_t page_zero_bytes = PGSIZE - page_read_bytes;
			...
			if(!insert_vme(&thread_current()->vm, vme)) {
				printf("insert vme error occured\n");
				return false;
			}
...
setup_stack (void **esp) 
{
  ...
  if (kpage != NULL) 
	...
	//additional implementation for virtual memory
	struct vm_entry *vme = (struct vm_entry*)malloc(sizeof(struct vm_entry));
	vme->type = VM_ANON;
