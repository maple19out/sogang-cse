...
{
	intr_disable();
	int value = fp_to_int_round(100*load_avg);
	intr_enable();
  return value;
}
...
{
	intr_disable();
	int value = fp_to_int_round(100*thread_current()->recent_cpu);
	intr_enable();
  return value;
}

...
void mlfqs_priority(struct thread *t) {
	if(t!=idle_thread) {
		t->priority = (int_to_fp(PRI_MAX) - t->recent_cpu/4 - int_to_fp(t->nice*2));
		t->priority = fp_to_int(t->priority);
		t->priority = t->priority > PRI_MAX ? PRI_MAX : t->priority;
		t->priority = t->priority < 0 ? 0 : t->priority;
	}
}
...
void mlfqs_load_avg(void) {
	...
	int x1 = div_fp(int_to_fp(59), int_to_fp(60));
	int x2 = load_avg;
	int x3 = div_fp(int_to_fp(1), int_to_fp(60));
	int x4 = int_to_fp(ready_threads);

	load_avg = mult_fp(x1, x2) + mult_fp(x3, x4);
	if(load_avg<0)
		load_avg=0;
}
