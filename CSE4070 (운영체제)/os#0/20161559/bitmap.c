...
/*expand bitmap to size of original size + SIZE */
struct bitmap*
bitmap_expand (struct bitmap *bitmap, int size) {
	size_t old_size = bitmap_size(bitmap);
	size_t bit_cnt = old_size + size;

	//create expanded size of bitmap
	struct bitmap *b = bitmap_create(bit_cnt);
	if(b==NULL)
		return NULL;

	//duplicate every element of original bitmap to new bitmap
	for(size_t idx=0; idx<old_size; idx++)
		bitmap_set(b, idx, bitmap_test(bitmap, idx));

	//deallocate original bitmap
